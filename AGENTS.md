...
- Before adding a new context directory to the registry, consider whether a **context flag** (`sew--{name}.yaml` patch file) on an existing context would be more appropriate. Flags are preferred for optional components that can be toggled without changing the fundamental nature of the deployment. See CONTRIBUTING.md for guidance on when to use flags vs separate contexts.

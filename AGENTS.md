...
- Concrete context READMEs must be **self-contained**. A reader who has never used sew should be able to go from zero to a running cluster using only the README. Never link to an abstract parent's README — abstract contexts don't get pages on the site and those links will be broken. Inline any relevant documentation from the parent directly into each concrete variant's README.
- Every concrete context README must include an **`## Install sew`** section (before `## Usage`) so a newcomer can get started immediately. Use the following template:
  ...
  For DNS-enabled contexts, append the DNS setup instructions (see the DNS rule above) to the same `## Install sew` section rather than creating a separate one.
- The **`## Usage`** section must include both the `sew create` command under a `### Create` subtitle and a `### Cleanup` block showing `sew delete`:
  ...
  \```
  ```
- Every concrete context README must include a **`## Quick Start`** section that tells users what to do right after `sew create`. For datastores, show the connection command and credentials. For applications, show the login URL and default credentials, and link to the product's getting started guide. Do **not** add `## Details`, `## Host access`, `## Dependencies`, or `## Context flags` sections — these are either internal implementation details or auto-generated by the site.

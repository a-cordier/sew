...
- After changing any file under `registry/` or `site/` (layouts, templates, CSS, config, generator), run `task site:build` before committing. The generated files in `site/content/registry/` and `site/static/` are tracked and must match their sources. `site/public/` is gitignored and rebuilt by CI.

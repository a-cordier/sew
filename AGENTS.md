...
- After changing any file under `registry/` or `site/` (layouts, templates, CSS, config, generator), run `task site:generate` before committing. The generated files in `site/content/registry/` and `site/static/` are tracked and must match their sources. If you are also changing layouts, templates, or CSS, run `task site:serve` to visually verify the site renders correctly. `site/public/` is gitignored (rebuilt by CI).
